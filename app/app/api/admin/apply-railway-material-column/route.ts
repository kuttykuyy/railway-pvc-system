...
const PENDING_EXTRAS: Array<{
  ...
  {
    label: 'bill_deletions',
    sql: (s) => `CREATE TABLE IF NOT EXISTS "${s}"."bill_deletions" (
        "id" BIGSERIAL PRIMARY KEY,
        ...
        "totalPvc" DOUBLE PRECISION,
        "dateOfMeasurement" TIMESTAMP(3),
        "billCreatedAt" TIMESTAMP(3),
        "ageMinutes" INTEGER
      )`,
    why: 'Bills are hard-deleted; this is the record that stays — who deleted what, whether it was the free-trial bill, and how long it lived — so a create-and-delete pattern can be seen instead of guessed at.',
    check: { kind: 'table', name: 'bill_deletions' },
  },
  {
    label: 'bill_deletions_deletedAt_idx',
    sql: (s) => `CREATE INDEX IF NOT EXISTS "bill_deletions_deletedAt_idx" ON "${s}"."bill_deletions" ("deletedAt" DESC)`,
    why: 'The admin page lists deletions newest first.',
    check: { kind: 'index', name: 'bill_deletions_deletedAt_idx' },
  },
  {
