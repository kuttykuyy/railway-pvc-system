...
export async function runScannedOcr(
  ...
  for (;;) {
    ...
    const data = (await res.json()) as {
      status: string; text?: string; numTables?: number; elapsedSec?: number; error?: string;
    };
    if (data.status === 'done') {
      onProgress('Read complete. Reading the fields…');
      return { text: data.text || '', numTables: data.numTables || 0, elapsedSec: data.elapsedSec || 0 };
    }
    if (data.status === 'error') throw new OcrError(data.error || 'The scan could not be read.');
