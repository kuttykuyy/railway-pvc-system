...
**CSP rollout — now ENFORCED.** A full `Content-Security-Policy` is live in `app/next.config.js`: `default-src 'self'`, a `script-src` that keeps `'unsafe-inline'`/`'unsafe-eval'` (required by Next.js + AdSense) while allowlisting Razorpay and the Google/AdSense delivery chain (googlesyndication, googleadservices, googletagservices, doubleclick, google.com, gstatic, googletagmanager, google-analytics, adtrafficquality), plus `object-src 'none'`, `base-uri 'self'`, `frame-ancestors 'none'`. **Verified in-browser:** the home page (which loads AdSense) renders and hydrates, all scripts including the AdSense loader are present, and there are **no CSP "Refused" violations**. Residual risk is limited to ad rendering — if a future ad/Razorpay resource is ever blocked, the browser console will name the origin; add it to the allowlist in `next.config.js` (a one-line change). The allowlists are intentionally broad so this is unlikely.
